...
Format: `type:source:destination`. Both source and destination support `~` for `$HOME` and environment variables (`$VAR`, `${VAR}`). The special shell built-in variables `UID`, `EUID`, `GID`, `EGID` are also expanded.
...
Shell rc files (`~/.bashrc`, `~/.bash_profile`, etc.) are **not** bound by default. Coding agents invoke tools directly and do not source shell init files. If your agent runs shell commands that need custom aliases, functions, or environment setup, add the relevant files to your config:

```
ro-bind:~/.bashrc:~/.bashrc
```

Note: since `--clearenv` is applied, even bound rc files won't be sourced automatically — the agent would need to explicitly source them. Most coding agents don't need this.
